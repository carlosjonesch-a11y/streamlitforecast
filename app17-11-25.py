...
try:
    if uploaded_file is not None:
        ...
        if tem_hora:
            ...
            if force_daily:
                granularidade = st.radio("Granularidade", ['Diária', 'Horária', 'Turnos'], index=0)
            else:
                granularidade = st.radio("Granularidade", ['Diária', 'Horária', 'Turnos'], index=default_index)
        else:
            granularidade = st.radio("Granularidade", ['Diária', 'Horária', 'Turnos'], index=0)
        ...
        if granularidade == 'Diária' and tem_hora:
            ...
            # Se os dados forem horários, oferecemos a opção de agregar por turnos
            # dentro da opção 'Diária' como alternativa ao uso da granularidade 'Turnos'.
            if tem_hora:
                # Mostrar expander de agregação por turno diretamente quando Granularidade == 'Diária' e houver componente horario
                with st.expander('Agregação por turno (apenas para dados horários) — Diária', expanded=False):
                        n_turnos = st.number_input('Número de turnos por dia (máx. 3)', min_value=1, max_value=3, value=2, key='n_turnos_diaria')
                        turnos = []
                        for i in range(int(n_turnos)):
                            c1, c2 = st.columns(2)
                            with c1:
                                s = st.slider(f'Turno {i+1} - Hora início', 0, 23, 11 + i*6, key=f'start_diaria_{i}')
                            with c2:
                                e = st.slider(f'Turno {i+1} - Hora fim', 0, 23, 16 + i*6, key=f'end_diaria_{i}')
                            turnos.append((int(s), int(e)))

                        agg_turno_d = st.selectbox('Agregação por turno (apenas para dados horários)', ['Soma', 'Média', 'Máximo', 'Mínimo', 'Percentil 80%'], index=0, key='agg_turno_diaria')

                        if st.button('Aplicar agregação por turno (Diária)'):
                            # Agregar dados por turno sem alterar granularidade para Turnos
                            df_para_turno = df.copy()
                            # Se houver coluna 'hora', construir timestamp a partir de 'data' + 'hora'.
                            if 'hora' in df_para_turno.columns:
                                ...
                                # Caso onde 'data' já contém componente horário (timestamp completo)
                                df_para_turno['data'] = pd.to_datetime(df_para_turno['data'], errors='coerce')
                            df_turnos = aggregate_shifts(df_para_turno, turnos, agg=agg_turno_d)
                            if df_turnos is None:
                                ...
                                if not valido:
                                    ...
                                    dados_sel = resultado
                                    st.success('Dados agregados por turno (Diária).')
