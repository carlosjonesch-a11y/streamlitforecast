...
# ----------------------
# Proteção simples por senha (opcional)
# ----------------------
def _check_password_from_secrets(secret_key='ADMIN_PW_HASH'):
    """Checa se existe uma senha (hash SHA256) em st.secrets.
    Se existir, pede que o usuário informe a senha e compara o hash. Se não bater,
    o app é interrompido com st.stop().

    Coloque o hash SHA256 (hex) em Streamlit Cloud: 'ADMIN_PW_HASH' e o app
    exigirá a senha.
    """
    try:
        # st.secrets funciona local se você tiver .streamlit/secrets.toml
        pw_hash = st.secrets.get(secret_key) if hasattr(st, 'secrets') else None
    except Exception:
        pw_hash = None

    # Não ativa se não houver segredo configurado
    if not pw_hash:
        return True
...
# Chamar a checagem no começo do fluxo de UI — só ativa quando ADMIN_PW_HASH estiver setado
_check_password_from_secrets(secret_key='ADMIN_PW_HASH')
